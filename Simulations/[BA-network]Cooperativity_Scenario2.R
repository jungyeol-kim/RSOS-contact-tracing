...
mainDir.6<-file.path(mainDir.5, paste0('Cooperativity (Scenario 2) ',cooperativity,'%'))
...
print(system.time({
  for(aaaa in c(4)){
    for(cccc in c(0.1,0.15,0.2,0.25,0.3)){
      topology<-paste0(mainDir.0,"/ba_network_k",aaaa,".RData")
      edges.all.undir.final<-get(load(topology))
      
      subDir<-paste0(aaaa," neighbors : PoI ",cccc)
      ...
      epidemic<-function(iteration,hhh){
          ...
          remove.edges<-vector(mode='list',length=T.max)
          remove.edges.non.coop<-vector(mode='list',length=T.max)
          edges.all.final.transmission[[1]]<-rbindlist(list(edge.list.undir[,c(1,2)],edge.list.undir[,c(2,1)]),use.names=FALSE)
          remove.edges.non.coop[[1]]<-edge.list.undir[(node1 %fin% non.cooperative.nodes | node2 %fin% non.cooperative.nodes)][['edge_num']]
          test.rt.all<-vector()
          ...
          for(t in 2:T.max){
            ...
            test<-unique(c(test.ct,test.rt)) # nodes who test today
            wdata.test.count[t]<-length(test) # the number of tests today
            
            temp<-vector(); P.list<-vector(); N.list<-vector(); FP.list<-vector(); FN.list<-vector(); TP.list<-vector(); TN.list<-vector(); test.rt.tp<-vector(); test.ct.tp<-vector();
            ...
            test.rt.tp<-test.rt[test.rt %fin% c(TP.list,FP.list)]
            test.ct.tp<-test.ct[test.ct %fin% c(TP.list,FP.list)]
            ...
            if(length(qt.nodes)>0){
              wdata.qt.count[t]<-length(qt.nodes)
            }
            
            
            #### Contact Tracing ####
            ## Contact network on day t
            temp.rm<-vector(); temp.rm.non.coop<-vector(); temp.edge<-vector(); 
            temp.rm<-which(wdata.qt.temp<0) # people who have died or are in quarantine
            temp.rm.non.coop <- unique(c(temp.rm,non.cooperative.nodes)) # people who have died or are in quarantine + non-cooperative people 
                                                                         # Health authority cannot find the information on contacts they are involved in.
            if(length(temp.rm)>0){
              remove.edges[[t]]<-edge.list.undir[(node1 %fin% temp.rm | node2 %fin% temp.rm)][['edge_num']]
              temp.edge<-edge.list.undir[!(edge_num %fin% remove.edges[[t]]),c(1,2)]
            }else{
              temp.edge<-edge.list.undir[,c(1,2)]
            }
            edges.all.final.transmission[[t]]<-rbindlist(list(temp.edge,temp.edge[,c(2,1)]), use.names=FALSE)  # actual contact network on day t
            
            if(length(temp.rm.non.coop)>0){
              remove.edges.non.coop[[t]]<-edge.list.undir[(node1 %fin% temp.rm.non.coop | node2 %fin% temp.rm.non.coop)][['edge_num']]
            }

            ## tracable network on day t
            TP.list.for.CT<-vector();
            TP.list.for.CT<-which(wdata.tp.new[[as.character(t)]]=='TP') # people who get positive test results on day t
            if(length(TP.list.for.CT)>0){
              wdata.tp.temp[TP.list.for.CT]<-1
              if((sum(wdata.inf.count[1:t])/N)>=(intervention.start.inf.rate/100.)){  # start contact tracing when the percentage of infected individuals exceeds a certain threshold.
                wdata.ct.day[t]<-1 # whether health authority perform contact tracing on day t
                
                if(hops>0){
                  tt<-vector(); temp.rm.all<-vector(); edges.ct.undir<-vector(); edges.ct.all<-vector();
                  tt<-ifelse(t-13>1,t-13,1):(t)            
                  
                  temp.rm.all<-unlist(remove.edges[tt])
                  temp.rm.all<-which(tabulate(temp.rm.all)==length(tt)) # edges which have not appeared for entire last 14 days. 
                  
                  temp.rm.all.non.coop<-unlist(remove.edges.non.coop[tt])
                  temp.rm.all.non.coop<-which(tabulate(temp.rm.all.non.coop)==length(tt)) # edges which have not been identified by health authority for entire last 14 days.
                  
                  
                  if(length(temp.rm.all.non.coop)>0){
                    edges.ct.undir<-edge.list.undir[!(edge_num %fin% temp.rm.all.non.coop),c(1,2)]
                  }else{
                    edges.ct.undir<-edge.list.undir[,c(1,2)]
                  }
                  edges.ct.all<-rbindlist(list(edges.ct.undir,edges.ct.undir[,c(2,1)]), use.names=FALSE) # edges which have been identified for contact tracing by health authority 
                  

                  
                  multihop<-vector(); QT.available<-vector()
                  multihop<-contact.tracing(hops,TP.list.for.CT,edges.ct.all) # trace contacts
                  QT.available<-which(wdata.qt.temp>=0 & wdata.tp.temp==0)  # people who can potentially be informed that they are in close contact with an infected person
                                                                            # people 1) who are alive & 2) who are not in quarantine & 3) who have not tested positive
                  QT.available<-QT.available[!QT.available %fin% non.cooperative.nodes] # people who will comply with the health authority's request among them
                                                                                        # non-cooperative nodes are excluded
        ...
        if(k==0){
          ...
          while(pp<=num_iteration){
            ...
            if(is.na(temp$iterationnum) == TRUE){
              print(paste('<k>=',aaaa,' : PoI=', cccc, ' hop ', k, ' iteration ', qq,  ' | ', temp$iterationnum, sep = ''))
              qq<-qq+1
            }else{
          ...
          for(pp in 1:num_iteration){
            ...
            daily.quarantines.hop[[pp]] <- (temp$dailyquarantines)
            contact.tracing.day.hop[[pp]]<-(temp$contacttracingday)
            print(paste('<k>=',aaaa,' : PoI=', cccc, ' hop ', k, ' iteration ', iteration.index[pp],  ' | ', temp$iterationnum, sep = ''))
      ...
      print(paste('<k>=',aaaa,' : PoI=',cccc, sep = ''))
      rm(list=ls()[! ls() %fin% c("aaaa","cccc","mainDir.0","mainDir.6","intervention.start.inf.rate","FN.rate","FP.rate","turnaround",'cooperativity','infness.of.asymp',
